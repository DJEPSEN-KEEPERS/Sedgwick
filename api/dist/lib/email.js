...
function sendNotificationEmail(params) {
    if (!CONNECTION_STRING || !SENDER)
        return;
    _sendNotification(params).catch((err) => console.error('[email] Failed to send notification e-mail:', err));
}
async function _sendNotification(p) {
    const client = new communication_email_1.EmailClient(CONNECTION_STRING);
    const message = {
...
function sendPasswordResetEmail(params) {
    ...
    _sendReset(params).catch((err) => console.error('[email] Failed to send password reset e-mail:', err));
}
/**
...
function sendWelcomeEmail(params) {
    ...
    _send(params).catch((err) => console.error('[email] Failed to send welcome e-mail:', err));
}
// ── Internal ──────────────────────────────────────────────────────────────────
...
// ── Password reset ────────────────────────────────────────────────────────────
async function _sendReset(params) {
    const client = new communication_email_1.EmailClient(CONNECTION_STRING);
...
// ── Plain-text fallback ───────────────────────────────────────────────────────
function buildResetPlainText(p) {
    return [
